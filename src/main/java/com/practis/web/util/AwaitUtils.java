...
public class AwaitUtils {
    ...
    public static ElementsCollection awaitElementCollectionSize(
            final int seconds,
            final Callable<ElementsCollection> callable,
        ...
        final var elements = callable.call();
        log.warn(
                "Element collection size is not {}. Cuttent size: {}. Wait time: {}",
                expectedSize,
                elements.size(),
                waitTime);
        return elements;
    ...
    /** Awaits given number of seconds until callable execution is true. */
    @SneakyThrows
    public static GridRow awaitGridRowExists(final int seconds, final Callable<GridRow> callable) {
        final var startTime = currentTimeMillis();
        var timeout = seconds * SECONDS_TO_MILLIS_MULTIPLIER;
        var waitTime = 0L;
        while (waitTime < timeout) {
